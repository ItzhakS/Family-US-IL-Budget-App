...
export const MaaserSummaryPanel: React.FC<{
  ...
  return (
    <div className="space-y-4">
      <div
        className={`p-4 rounded-xl border ${currentBalance > 0 ? 'bg-amber-50 border-amber-100' : 'bg-green-50 border-green-100'}`}
      >
        ...
        <div className="flex items-baseline gap-2">
          ...
          <span
            className={`text-xs font-medium uppercase tracking-wide ${currentBalance > 0 ? 'text-amber-600' : 'text-green-600'}`}
          >
              ...
              {monthlyStats.map((stat) => (
                <React.Fragment key={stat.month}>
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-right bg-gray-50 font-medium whitespace-nowrap">
                      {stat.obligation.toLocaleString()}
                    </td>
                    <td className="px-3 py-2 text-right text-pink-600 whitespace-nowrap">
                      {stat.paid.toLocaleString()}
                    </td>
                    <td
                      className={`px-3 py-2 text-right font-bold whitespace-nowrap ${stat.runningBalance > 0 ? 'text-amber-600' : 'text-green-600'}`}
                    >
                        ...
                        <div className="text-xs">
                          ...
                          <ul className="space-y-1">
                            {stat.deductibleTransactions.map((t) => (
                              <li
                                key={t.id}
                                className="flex justify-between text-gray-600 border-b border-amber-100 pb-1 last:border-0"
                              >
